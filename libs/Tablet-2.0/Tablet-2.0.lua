...
local MINOR_VERSION = "$Revision: 23368 $"
...
local tooltip
local testString-- = CreateFrame("Frame", UIParent):CreateFontString(nil, "OVERLAY")
local TabletData = {}
local Category = {}
local Line = {}
local function getTestWidth(font, size, text)
	...
	local a,_,b = testString:GetFont()
...
do
	...
	function TabletData:checkMinWidth()
		local width = (MIN_TOOLTIP_SIZE - 20)*self.tablet.fontSizePercent
...
do
	...
	function Line:new(category, info, position)
		...
		local w = 0
		self.checkWidth = 0
		testString = category.tabletData.tablet.lefts[1]
		if self.text then
		...
		elseif columns >= 3 then
			...
			if self.text3 then
				if not self.wrap3 then
					local testWidth = getTestWidth(self.font3, self.size3 * fontSizePercent, self.text3)
					local w = testWidth
			...
			if columns >= 4 then
				if self.text4 then
					if not self.wrap4 then
						local testWidth = getTestWidth(self.font4, self.size4 * fontSizePercent, self.text4)
						w = testWidth
				...
				if columns >= 5 then
					if self.text5 then
						if not self.wrap5 then
							local testWidth = getTestWidth(self.font5, self.size5 * fontSizePercent, self.text5)
							w = testWidth
					...
					if columns >= 6 then
						if self.text6 then
							if not self.wrap6 then
								local testWidth = getTestWidth(self.font6, self.size6 * fontSizePercent, self.text6)
								w = testWidth
...
local function NewLine(self)
	if self.maxLines <= self.numLines then
		...
		local button = CreateFrame("Button", "Tablet20Button" .. num_buttons, self)
...
local function GetMaxLinesPerScreen(self)
	if self.height_ then
		...
		height = height - 20
		return floor(height / (12*self.fontSizePercent+2))
	end
...
local function AcquireFrame(self, registration, data, detachedData)
	...
	if tooltip then
		...
		function tooltip:Show(tabletData)
			...
			testString = self.lefts[1]
			
			local maxWidth = tabletData and tabletData.width or self:GetWidth() - 20
			local hasWrap = false
			local screenWidth = GetScreenWidth()
			local scrollMax = self.numLines
			local maxLinesPerScreen = GetMaxLinesPerScreen(self)
			if scrollMax > maxLinesPerScreen + self.scroll then
				scrollMax = maxLinesPerScreen + self.scroll
			end
			local numColumns
...
--			testString = left
			...
			do -- if columns >= 1 then
				...
				left:SetFont(font, info.size * fontSizePercent, flags)
				left:SetText(text)
				...
				if columns < 2 then
					...
					if columns < 3 then
						...
						right:SetPoint("TOPLEFT", left, "TOPRIGHT", (20 - info.indentation) * fontSizePercent, 0)
			...
			elseif columns == 3 then
				...
				right:SetPoint("TOPLEFT", left, "TOPRIGHT", num - info.indentation * fontSizePercent, 0)
		...
		function tooltip:Scroll(down)
			...
			if down then
				if IsShiftKeyDown() then
					self.scroll = self.numLines - maxLinesPerScreen
				else
			...
			if self.scroll > self.numLines - maxLinesPerScreen then
				self.scroll = self.numLines - maxLinesPerScreen
			end
			if self.scroll < 0 then
				self.scroll = 0
...
function AcquireDetachedFrame(self, registration, data, detachedData)
	...
	for _, detached in ipairs(detachedTooltips) do
		if detached.notInUse then
			...
			detached:SetParent(registration.parent or UIParent)
	...
	local detached = CreateFrame("Frame", "Tablet20DetachedFrame" .. (#detachedTooltips + 1), UIParent)
	...
	detached:SetScript("OnDragStart", function()
		detached:StartMoving()
		detached.moving = true
	end)

	detached:SetScript("OnDragStop", function()
...
--	self:Close(fakeParent)
	...
	function frame:children()
		if not self.preventRefresh and self:GetParent():IsVisible() then
			currentFrame = self
			currentTabletData = TabletData:new(self)
--			if info.width then
--				currentTabletData.width = info.width
--			end
			self:ClearLines()
			if children then
				children()
...
function Tablet:SetHint(text)
	...
	if currentCategoryInfo then
		self:error("You cannot set hint in a category.")
	end
...
function Tablet:SetTitle(text)
	...
	if currentCategoryInfo then
		self:error("You cannot set title in a category.")
	end
...
function Tablet:SetTitleColor(r, g, b)
	...
	if currentCategoryInfo then
		self:error("You cannot set title color in a category.")
	end
